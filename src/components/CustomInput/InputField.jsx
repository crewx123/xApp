...
const InputField = ({ labelName, labelNameColor, labelFontsize = 14, name, setName, textInputColor = '#161D23', textInputPlaceHolderColor = '#666', textInputBorderWidth = 1, textInputBorderColor = '', textInputFontSize = 16, textInputPaddingHorizontal = 16, inputType = 'default', iconColor, inputPlaceholder, textInputBackgroundColor = '', iconName, maxInputSize = undefined, autoCapitalize = 'none', errorName = null, pickerList = [], isRequired = false, multilineValue = false, numberOfLinesValue = 1, radioValues = [] }) => {
    ...
    return (
        <View>
            <View style={{ flexDirection: 'row', gap: 2 }}>
                <Text style={{ ...Styles.eachInputLabel, color: labelNameColor ? labelNameColor : '#161D23', fontSize: labelFontsize }}>{labelName}</Text>
                {isRequired && <Text style={{ color: 'red' }}>*</Text>}
            </View>
            <View style={{ ...Styles.inputContainer, marginBottom: errorName ? 0 : 10, paddingRight: inputType === 'picker' ? 8 : 20, backgroundColor: textInputBackgroundColor ? textInputBackgroundColor : '#fff', borderWidth: textInputBorderWidth, borderColor: textInputBorderColor, paddingHorizontal: textInputPaddingHorizontal }}>
